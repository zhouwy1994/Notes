...
封装：封装实现了类的接口和实现的分立,封装后的类隐藏了它实现的细节，也就是说，类
的用户只能使用接口而无法访问实现部分
...
短路运算
C++ 随机数
在许多情况下，需要生成随机数。关于随机数生成器，有两个相关的函数。一个是 rand()，该函数只返回一个伪随机数。生成随机数之前必须先调用 srand() 函数。由于c的随机数采用
一个全局标志位生成，所以srand志需要
...
使用初始化列表来初始化字段：适用于初始化const类型变量，引用等

Line::Line( double len): length(len)
如果在类中没有定义拷贝构造函数，编译器会自行定义一个。如果类带有指针变量，并有动态内存分配，则它必须有一个拷贝构造函数。(默认浅拷贝)
...
浮点型可用指数形式E或e e表示10 
字面值修饰：前缀：u： char16_t U: char32_t  L: wchar_t (要加前缀)
后缀：U or u unsigned
L or l long
ll or LL long long
F or f float
...
如果不是常量，两个都可使用。
cbegin()和cend()返回iterator和const_iterator；类型
...
raw string:c++中原始字符串使用使用前缀R"()"表示，将不用转义字符
...
2.静态数据成员可以是当前类类型，也可以是当前类指针，而普通非静态成员不可以定义成当前类类型
class CBook{
	public:
		static CBook cb; //允许
		CBook cb1;//不允许
...
3.静态成员函数能作为其他成员函数的初始化参数，而普通成员函数不能
4.静态成员函数可以访问类的构造函数****
class CBook{
	public:
		static int a;
...
面向对象的三个特征:封装、继承、多态
面向对象的四个优点:可维护、可扩展、可复用、灵活性好(可参照:活体印刷术)
...
当子类和父类存在同名成员函数，子类的函数会覆盖父类的成员函数，以及父类的所有函数重载(隐藏)
...
虚继承对子类没什么影响，但对于子类的子类有影响，虚继承保证了子类的子类只存在一份祖类的副本
...
答案：都是在堆(heap)上进行动态的内存操作(new 可能会在静态存储区分配)。用malloc函数需要指定内存分配的字节数并且不能初始化对象，new 会自动调用对象的构造函数。delete 会调用对象的destructor，而free 不会调用对象的destructor.
...
拷贝构造函数:显式调用构造函数、函数传参、函数返回值、class object = object1时被调用，一直以为拷贝构造函数是先调用默认构造函数再电泳拷贝构造函数，
其实不论是拷贝构造函数还是默认构造函数，一个对象的生命周期内，都只调用其中一个构造函数构造，而其只调用一次，所以如果有head上的数据，在拷贝构造函数内别忘记分配。

nullptr 是C++11新加入的关键字表示空指针类型，而在c++中NULL代表0,如果是函数重载，NULL将被解析成0
