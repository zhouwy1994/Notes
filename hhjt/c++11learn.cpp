...
71.是时候好好梳理一下linux下gcc的头文件、静态库、动态库默认搜索路径
...
72.ratio(n. 比率，比例)之前还以为是什么高深的数据结构,其实就是一个分数的数据结构,在头文件ratio中,该头文件里
...
73.sprintf snprintf库函数有一个返回值(size_t),含义是写入buf的字符实际长度,为了防止越界,应该使用snprintf，
虽然此函数不会访问越界，但有时实际写入长度大于buf长度时不时我们想要的结果，这是就需要对snprintf的返回值进行判断

74.c++可以获取更高精度的时间戳，之前一直都没好好看懂chrono，clock,duration,time_point,tick,epoch,ratio
duration.count()
